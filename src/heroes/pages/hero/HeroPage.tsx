...
export const HeroPage = () => {
  const { idSlug = "" } = useParams(); //Para resivir el argumento/parametro de la url. Esto es antes del ? mientras que useSearchParams es después del ?
  console.log(idSlug);
  const { data: hero, isError } = useQuery({
    queryKey: ["Hero-information", { idSlug }],
    queryFn: () => getHeroAction(idSlug),
    staleTime: 1000 * 60 * 5,
    retry: false, //Tanstak query hace hasta 3 intentos por defecto, con esto, si el primer intento falla ya no hace más
  });
